...
- Windows keyboard muscle memory: F2 rename, F5 refresh, Alt+arrows for navigation, Backspace for up,
  Shift+Delete to trash, Alt+Enter for Properties, Ctrl-style shortcuts on the Command key
...
```sh
.build/debug/Explorerr --selftest    # headless logic checks (30 checks, no XCTest needed)
...
| Rename | F2 |
| Move to Recycle Bin | Shift-Delete (or fn-Delete) |
| Go up (parent folder) | Backspace or Option-Up |
| Back / Forward | Option-Left / Option-Right |
...
| Properties of selection | Cmd-I or Option-Return |
| Quick Look | Space or Cmd-Y |
| Details pane | Shift-Cmd-I |
| Find (focus the search box) | Cmd-F |
| Open file or folder | Return |
| Select next / previous item | Up / Down arrows |
...
| Toggle integrated terminal | F4 or Shift-Cmd-T |
...
│   └── Views/       folder content views, navigation pane, Home, This PC, Gallery,
│                    Trash, Network, terminal panel, dialogs
scripts/             make-app.sh, verify.sh, gen-icon.swift
